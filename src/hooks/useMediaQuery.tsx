...
const MediaQueryBreakPoint = {
  ...
  tablet: "(min-width: 459px) and (max-width: 1023px)",
  mobile: "(max-width: 458px)",
  // mobileL: "(min-width: 360px) and (max-width: 673px)",
  mobileS: "(max-width: 359px)",
};
...
export const MediaQueryProvider = ({ children }: { children: ReactNode }) => {
  ...
  return (
    <MediaQueryContext.Provider value={contextRef.current}>{children}</MediaQueryContext.Provider>
  );
};
...
const useMediaQuery = (breakPoint: keyof BreakPoint) => {
  ...
  const [isMatch, setIsMatch] = useState(
    context.mediaQueryMatchesRef?.current[mediaQuery] || false
  );
